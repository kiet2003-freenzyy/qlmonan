...
<body>
    ...
    <form method="get" action="Bai5.php">   
        ...
        <div id="Content" class="row" style="margin-top:5px;">
            ...
            <!--  Content
            -->  
            <h2 align="center" style="color:#900;">THÔNG TIN CHI TIẾT MÓN ĂN </h2>
            	<?php foreach($mon AS $m)
                    { ?>
                    <div class="dd_mon"><img src="image_MonAn/<?php echo $m[8]; ?>" width="60%" style="float:left;margin:20px;"/>
                        <div style="clear:right; margin:20px;">
                            <p style="font-weight:bold;">Món: <?php echo $m[2];?></p>
                            <p >Giá: <?php echo $m[5]." - VNĐ / ".$m[10];?></p>
                            <p > Giá KM: <?php  echo $m[6]." - VNĐ";?></p>
                            <p >Khuyến Mãi: <span style="color:red"><?php echo $m[7];?></span></p>
                            <form class="form-inline" method="POST" action="ShowCart.php">
                                <div class="form-group mb-2" style="margin-right:10px;">
                                    <input type="hidden" name="maMon" value="<?php echo $m['ma_mon']?>">
                                    <input type="hidden" name="hinh" value="<?php echo $m['hinh']?>">
                                    <input type="hidden" name="tenMon" value="<?php echo $m['ten_mon']?>">
                                    <input type="hidden" name="donGia" value="<?php echo $m['don_gia']?>">
                                    <input type="number" name="sl" id="productQty" class="form-control" placeholder="Quantity" min="1" max="1000" value="1" size="7" style="margin-right:30px;">
                                    
                                
                                    <button type="submit" class="btn btn-primary" style="float:right;" name="add_to_cart" value="add to cart">CHO VÀO GIỎ</button>
                                </div>
                            </form>
